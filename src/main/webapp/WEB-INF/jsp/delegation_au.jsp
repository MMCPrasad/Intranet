                        ...
                        <div class="card" >
                            ...
                            <div class="card-footer">
                                <div class="text-right">
                                    <button id="addDeleBtn" class="btn btn-sm waves-effect waves-light btn-danger"><i class="icon feather icon-plus"></i>Add a Delegation Authority</button>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $(document).on('click', '.delrec', function () {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: "This Delegation Authority Will be Deleted!",
            icon: 'warning',
            ...
            preConfirm: () => {
                return fetch('delegation/deactivate-delegation', {
                    method: 'POST',
            ...
            if (result.value) {
                if (result.value.status !== 200) {
                    ...
                    Swal.fire('Successfull!', 'Delegation Authority has been Deactivated !', 'success');
                    dtable.ajax.reload();
                    $('#formSection').hide();
                    $('#tableSection').fadeIn();
                }
    ...
    $(document).on('click', '.rerec', function () {
        ...
        Swal.fire({
            title: 'Are you sure?',
            text: "Delegation Authority Will be Activated!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            showLoaderOnConfirm: true,
            preConfirm: () => {
                return fetch('delegation/reactivate-delegation', {
                    method: 'POST',
                    body: new URLSearchParams({
                        id: id
                    })
            ...
            if (result.value) {
                if (result.value.status !== 200) {
                    ...
                    Swal.fire('Successfull!', 'Delegation Authority has been Activated !', 'success');
                    dtable.ajax.reload();
                    $('#formSection').hide();
                    $('#tableSection').fadeIn();
                }
...
// Add a function to clear the form fields
    ...
    document.getElementById('saveBtn').addEventListener('click', function () {
        ...
        if (mode === 'save') {
            ...
            let delegation = new FormData();
            let file = document.getElementById('file').files;
            let name = document.getElementById('name').value;
            delegation.append('name', name);
            for (var i = 0; i < file.length; i++) {
                delegation.append('file', file[i]);
            }
            fetch('delegation/save', {
                method: 'POST',
                body: delegation
            }).then(response => {
                if (!response.ok) {
                    ...
                    Swal.fire('Successful!', 'Delegation Authority has been successfully saved', 'success');
                    clearForm();
                    $('#formSection').hide();
                    $('#tableSection').fadeIn();
                    dtable.ajax.reload();
            ...
            $.ajax({
                url: 'delegation/update', // Replace with the actual update endpoint
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function (response) {
                    if (response.status === 200) {
                        Swal.fire('Successful!', 'Delegation Authority details updated successfully', 'success');
                        clearForm();
                        $('#formSection').hide();
                        $('#tableSection').fadeIn();
                        dtable.ajax.reload();
                ...
                error: function (xhr, status, error) {
                    ...
                    console.log(status);
                    console.log(error);
                    Swal.fire('Error!', 'Failed to update Delegation Authority details', 'error');
                }
            });
        }
    });
